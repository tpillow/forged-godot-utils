	...
	for child in get_parent().get_children():
		if is_instance_of(child, type):
			assert(is_instance_of(child, StateNode),
				"goto_state_by_type was provided a type that does not inherit StateNode")
			return child
	return null
...
static func find_active_state(parent: Node, ignore_safety_check: bool = false) -> StateNode:
	var found_state_node := false
	for child in parent.get_children():
		if child is StateNode:
			found_state_node = true
			if (child as StateNode).is_active_state():
				return child
	assert(ignore_safety_check or not found_state_node,
